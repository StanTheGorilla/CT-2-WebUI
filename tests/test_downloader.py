...
def test_get_platform_info_returns_required_keys():
    from ct2.server.downloader import _get_platform_info
...
def test_find_asset_matches_by_substring():
    from ct2.server.downloader import _find_asset
    assets = [
...
def test_strip_quant_suffix_removes_trailing_quant_markers():
    from ct2.server.downloader import _strip_quant_suffix
...
def test_find_hf_repo_for_mmproj_prefers_exact_repo(monkeypatch):
    from ct2.server.downloader import _find_hf_repo_for_mmproj
    ...
    monkeypatch.setattr("ct2.server.downloader._fetch_json", fake_fetch)
...
def test_resolve_mmproj_download_returns_resolve_url(monkeypatch):
    from ct2.server.downloader import resolve_mmproj_download
...
def test_ensure_mmproj_downloaded_writes_missing_file(monkeypatch):
    ...
    monkeypatch.setattr(
        "ct2.server.downloader.resolve_mmproj_download",
        lambda _model_path: (
    ...
    monkeypatch.setattr("ct2.server.downloader._download_file", fake_download)
